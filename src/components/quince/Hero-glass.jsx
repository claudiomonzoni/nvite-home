...
import Style from "../../estilos/temas/glass/quince/hero.module.scss";

export default function Hero({ nombres, fecha, cover, labels, lang = 'es-ES', initialInvitado = null }) {
  const l = {
  ...
  useEffect(() => {
    ...
    const updateDOM = (nombre, pasesVal) => {
      ...
      if (paseSpanEl && spanVariosEl) {
        if (pasesVal > 1) {
          spanVariosEl.textContent = "Los";
          paseSpanEl.textContent = " pases";
        } else {
          spanVariosEl.textContent = "Te";
          paseSpanEl.textContent = " pase";
        }
      }
    };
    ...
    if (id && uid) {
      ...
      fetch(`${window.location.origin}/api/getInvitado.json?id=${id}&uid=${uid}`)
        ...
        .catch(err => console.error("Error fetching guest in Hero-glass:", err));
    }
  }, [initialInvitado]);
